...
class Author:
    ...
    def get_all(cls):
        ...
        authors = []
        results = connectToMySQL('books-schema').query_db(query)
        for row in results:
    ...
    def add(cls,data):
        ...
        return connectToMySQL('books-schema').query_db(query,data)

    @classmethod
    def unfavorited_authors(cls,data):
        query = "SELECT * FROM authors WHERE authors.id NOT IN ( SELECT author_id FROM favorites WHERE book_id = %(id)s );"
        authors = []
        results = connectToMySQL('books-schema').query_db(query,data)
        for row in results:
            authors.append(cls(row))
        return authors
